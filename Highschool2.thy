        ...
        proof (cases "G2 = B'")
          ...
          proof -
            ...
            proof -
              ...
              hence "Coplanar A B G1 G2" 
                by (metis Tarski_Euclidean.IsCircumcenter_def Tarski_Euclidean_axioms 
                    True \<open>B' Midpoint A C\<close> assms(4) bet_cop2__cop
                    coplanar_perm_8 midpoint_bet ncop_distincts)             
...
proof -
  ...
  proof -
    ...
    moreover
    have "Col A G I" 
      using MidR_uniq_aux \<open>A'' Midpoint B C\<close> \<open>Col G A A''\<close> \<open>G \<noteq> A\<close> 
        \<open>I Midpoint A' G'\<close> assms(2) assms(3) calculation(3) 
        col_transitivity_2 l6_16_1 midpoint_col
    proof -
      have f1: "A' \<noteq> G'"
        using \<open>A' = G' \<Longrightarrow> False\<close> by blast
      have f2: "Col I A' G'"
        using \<open>I Midpoint A' G'\<close> midpoint_col by blast
      have f3: "A' \<noteq> G' \<and> Col I A' G' \<and> Col G' A' G' \<longrightarrow> Col G' A' I"
        using l6_16_1 by blast
      have f4: "A' \<noteq> G' \<and> Col G' A' G' \<longrightarrow> Col A' G' G'"
        by (metis (no_types) col_transitivity_2)
      have f5: "I \<noteq> G' \<and> Col A'' G' I \<and> Col I G' I \<longrightarrow> Col I G' A''"
        by (metis (no_types) l6_16_1)
      have f6: "A'' \<noteq> G' \<and> Col I G' A'' \<and> Col G G' A'' \<longrightarrow> Col G G' I"
        by (metis (no_types) l6_16_1)
      have f7: "A' \<noteq> G' \<and> Col A' G' I \<or> \<not> Col A' G' I"
        using f1 by meson
      ...
      then have f10: "A'' = aa"
        using \<open>A'' Midpoint B C\<close> by blast
      have f11: "A' = aa"
        using f9 \<open>A' Midpoint B C\<close> by blast
      then have f12: "A'' \<noteq> G'"
        using f10 f1 by meson
      have f13: "\<not> Col A' G' I \<or> Col A'' G' I"
        using f11 f10 by meson
      { assume "Col I G' I"
        moreover
        { assume "I \<noteq> G' \<and> Col A'' G' I \<and> Col I G' I"
          then have "Col G G' A'' \<and> A = G' \<longrightarrow> Col G A I"
            using f12 f6 f5 by meson }
        ultimately have "Col G G' A'' \<and> A = G' \<and> Col A' G' I \<longrightarrow> Col G A I \<or> I = G'"
          using f13 by fastforce }
      moreover
      { assume "I = G' \<or> \<not> Col G' I G' \<or> \<not> Col G' I I"
        moreover
        { assume "\<not> Col G' I G'"
          moreover
          { assume "\<not> Col A' G' G'"
            then have "\<not> Col G' A' G'"
              using f4 by meson }
          ultimately have "\<not> Col A' G' I \<or> \<not> Col G' A' G'"
            using f1 col_transitivity_2 by blast }
        moreover
        { assume "\<not> Col G' I I"
          then have "\<not> Col A' G' I"
            using f8 f7 by meson }
        ultimately have "Col G' A' G' \<and> Col A' G' I \<longrightarrow> I = G'"
          by blast }
      moreover
      { assume "\<not> Col A' G' I"
        moreover
        { assume "\<not> Col G' A' I"
          then have "\<not> Col G' A' G'"
            using f3 f2 f1 by meson }
        ultimately have "\<not> Col G' A' G'"
          by (metis (no_types) col_transitivity_2) }
      ultimately show ?thesis 
        by (metis \<open>A' = A''\<close> \<open>Col G A A''\<close> assms(2) col_permutation_3
            col_permutation_4 colx f12 f2 midpoint_col)
    qed
