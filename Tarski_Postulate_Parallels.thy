...
section "Pour compatibilité géocoq"
...
definition Postulate04 :: "bool" where 
  ...
  (* dans le code Geocoq on a 
 Coplanar P Q R P1 \<and> Coplanar P Q R R1
que l'on a plus dans l'article "Parallel postulates and continuity axioms: a mechanized
study in intuitionistic logic using Coq" 
TODO
*)
...
definition Postulate31 :: "bool" where "Postulate31 \<equiv> weak_inverse_projection_postulate"
...
proof -
  {
    fix A B C D P Q R
    assume P1: "B C OS A D \<and> SAMS A B C B C D \<and> 
                A B C B C D SumA P Q R \<and> \<not> Bet P Q R"
    ...
    qed
    obtain A' where P30: "Bet C A' E \<and> (A' = B \<or> B Out A' A)" 
      using P28 InAngle_def by auto
    {
      assume "A' = B"
      hence "Col D' C B"
      ...
      hence "False"
        by (simp add: P20)
      hence "\<exists> Y. B Out A Y \<and> C Out D Y" by auto
    }
    {
      ...
      qed
      then obtain Y where P34: "Bet D' C Y \<and> BetS B A' Y" 
        using BetSEq by blast
      hence "\<exists> Y. B Out A Y \<and> C Out D Y"
      proof -
        have P35: "B Out A Y"
...
proof -
  {
    fix P Q R S T U
    assume
      P1: "BetS P T Q \<and> BetS R T S \<and> BetS Q U R \<and> 
      ...
      using BetSEq P1C by blast
    {
      assume "P = W"
      have "P \<noteq> V"
        by (metis BetSEq Bet_perm Col_def Cong_perm Midpoint_def 
            P1A P1B P1D P1E P1F P2 between_trivial is_midpoint_id_2 l7_9)
      have "Col P Q S"
      proof -
        have f1: "Col V P R"
          by (meson Col_def P3)
    ...
    proof -
      ...
      qed
      {
        ...
        hence W3: "Q = U" 
          using BetS_def Col_def P1C P7 W2 col_transitivity_2 
          by (metis Bet_cases)
        hence "False"
          using BetS_def P1C by auto
      }
      hence "\<not> Col Y Q S" by auto
      hence "Q S TS X Y"
...
proof -
    ...
    qed
    {
      ...
      proof -
        ...
        moreover
        {
          ...
          ultimately have "\<exists> C'. Col C1 C2 C' \<and> B1 B2 TS A1 C'" 
            using cop_not_par_other_side
            by (meson not_col_permutation_4)
        }
        ultimately show ?thesis 
          using \<open>\<not> Col C1 B1 B2 \<or> \<not> Col C2 B1 B2\<close> by blast
      qed
      then obtain C' where W1: "Col C1 C2 C' \<and> B1 B2 TS A1 C'" by auto
      hence W2: "\<not> Col A1 B1 B2"
        using TS_def by blast
      ...
      hence W4A: "Bet A1 B C' \<and> Bet C P C'"
  ...
  {
    fix A1 A2 B1 B2 P C1 C2
    assume "Col P A1 A2" and "A1 A2 Par B1 B2" and "Col P B1 B2" and
      "A1 A2 Par C1 C2" and "Col P C1 C2"
    have "C1 \<noteq> C2" 
      using \<open>A1 A2 Par C1 C2\<close> par_neq2 by blast
    have "Coplanar A1 A2 B1 B2" 
      using Coplanar_def NCol_perm \<open>Col P A1 A2\<close> \<open>Col P B1 B2\<close> by blast
    have "Coplanar A1 A2 C1 C2" 
      using \<open>A1 A2 Par C1 C2\<close> par__coplanar by auto
    have "Coplanar B1 B2 C1 C2" 
      using Coplanar_def \<open>Col P B1 B2\<close> \<open>Col P C1 C2\<close> not_col_permutation_2 by blast
    have "B1 B2 Par C1 C2"
    proof -
      have "C1 \<noteq> C2" 
        using \<open>A1 A2 Par C1 C2\<close> par_neq2 by blast
      moreover have "Coplanar A1 A2 B1 B2" 
        using Coplanar_def NCol_perm \<open>Col P A1 A2\<close> \<open>Col P B1 B2\<close> by blast
      moreover have "Coplanar A1 A2 C1 C2" 
        using \<open>A1 A2 Par C1 C2\<close> par__coplanar by auto
      moreover have "Coplanar B1 B2 C1 C2" 
        using Coplanar_def \<open>Col P B1 B2\<close> \<open>Col P C1 C2\<close> not_col_permutation_2 by blast
      moreover have "Col C1 C2 A2" 
        by (meson \<open>A1 A2 Par C1 C2\<close> \<open>Col P A1 A2\<close> \<open>Col P C1 C2\<close> 
            col_not_col_not_par col_permutation_1 col_trivial_2 par_symmetry)
      moreover have "Col C1 C2 A1" 
        by (meson \<open>A1 A2 Par C1 C2\<close> calculation(5) inter_uniqueness_not_par 
            not_col_distincts par_symmetry) 
      ultimately show ?thesis 
        using Par_cases \<open>A1 A2 Par B1 B2\<close> par_col2_par_bis by blast
    qed
    hence "Col C1 B1 B2" 
      using \<open>Col P B1 B2\<close> \<open>Col P C1 C2\<close> not_col_permutation_2 not_strict_par by blast
    moreover have "Col C2 B1 B2" 
      by (meson \<open>B1 B2 Par C1 C2\<close> \<open>Col P B1 B2\<close> \<open>Col P C1 C2\<close> 
          inter_uniqueness_not_par not_col_permutation_3 par_comm)
    ultimately have "Col C1 B1 B2 \<and> Col C2 B1 B2" by auto
  }
  ultimately show ?thesis
    using playfair_s_postulate_def by metis
qed
          ...
          moreover
          {
            ...
            proof -
              obtain pp :: "TPoint \<Rightarrow> TPoint \<Rightarrow> TPoint \<Rightarrow> TPoint \<Rightarrow> TPoint \<Rightarrow> TPoint" where
                ...
                by moura
              hence f1: "\<forall>p pa pb pc pd pe. p = pa \<or> 
                                \<not> Col pb pc pd \<or> \<not> Col p pa pd \<or> 
                                 Col pb pc p \<or> Col pb pc pe \<or> 
                                 \<not> Coplanar pb pc p pe \<or> 
                                 Col p pa (pp pe pc pb pa p) \<and> 
                                 pb pc OS pe (pp pe pc pb pa p)"
                using cop_not_par_same_side by force
              hence "Col C D (pp Q1 C0 P C D)"
                using \<open>Col D C C0\<close> \<open>Coplanar P C0 D Q1\<close> \<open>D \<noteq> C\<close> 
                  \<open>\<not> Col P C0 D\<close> \<open>\<not> Col P C0 Q1\<close> col_trivial_2 
                  col_trivial_3 colx by blast
              then show ?thesis
                using f1 \<open>Col D C C0\<close> \<open>Col P C0 C0\<close> 
                  \<open>Coplanar P C0 D Q1\<close> \<open>D \<noteq> C\<close> \<open>\<not> Col P C0 D\<close> 
                  \<open>\<not> Col P C0 Q1\<close> by blast
            qed 
          }
          ultimately
          show ?thesis
